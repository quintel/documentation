...
1. **CHPs**  
   CHPs are a special case since they produce both electricity and heat. As stated before there is no revenue attributed for heat production, only the revenue that is generated from electricity production is taken into account for CHPs.
...
3. **Battery parks & offshore hybrid wind parks**  
   These plants consist of multiple components. As a result, it is difficult to determine the economic viability of one of the components in the plant. 
   As a result, they are excluded from the tables.
